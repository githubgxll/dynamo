...
func TestSGLangBackend_PythonCommandInjection(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedArgs:      []string{"-m", "dingo.sglang"},
		...
		},
		{
			...
			multinodeDeployer: &MockSimpleDeployer{},
			initialCommand:    []string{"python3"},
			initialArgs:       []string{"-m", "dingo.sglang", "--model", "llama"},
			expectedCommand:   []string{"python3"},
			expectedArgs:      []string{"-m", "dingo.sglang", "--model", "llama", "--dist-init-addr", "leader.example.com:29500", "--nnodes", "2", "--node-rank", "1"},
		...
		},
		{
			...
			initialArgs:       []string{"-m", "dingo.sglang", "--model", "llama"},
			expectedCommand:   []string{"python3"},
			expectedArgs:      []string{"-m", "dingo.sglang", "--model", "llama", "--dist-init-addr", "$(LWS_LEADER_ADDRESS):29500", "--nnodes", "2", "--node-rank", "$(LWS_WORKER_INDEX)"},
		...
		},
		{
			...
			expectedArgs:      []string{"exec python3 -m dingo.sglang --model llama --dist-init-addr $(LEADER_HOST):29500 --nnodes 2 --node-rank $(WORKER_INDEX)"},
		...
		},
		{
			...
			expectedArgs:      []string{"-m", "dingo.sglang", "--dist-init-addr", "$(LEADER_HOST):29500", "--nnodes", "3", "--node-rank", "0"},
		...
		},
		{
			...
			expectedArgs:      []string{"-m", "dingo.sglang", "--dist-init-addr", "leader.example.com:29500", "--nnodes", "2", "--node-rank", "1"},
		...
		},
		{
			...
			initialArgs:       []string{"-m", "dingo.sglang"},
			expectedCommand:   []string{"sh", "-c"},
			expectedArgs:      []string{"exec /home/user/.pyenv/shims/python3.9 -m dingo.sglang --dist-init-addr $(LEADER_HOST):29500 --nnodes 2 --node-rank $(WORKER_INDEX)"},
		...
		},
		{
			...
			role:              RoleWorker,
			multinodeDeployer: &MockSimpleDeployer{},
			initialCommand:    []string{"python3", "-m", "dingo.sglang"},
			initialArgs:       []string{"--model-path", "Qwen/Qwen3-0.6B", "--tp-size", "8"},
			expectedCommand:   []string{"python3", "-m", "dingo.sglang"},
		...
		},
		{
			...
			expectedCommand:   []string{"sh", "-c"},
			expectedArgs:      []string{"exec python3 -m dingo.sglang --model-path Qwen/Qwen3-0.6B --dist-init-addr $(LEADER_HOST):29500 --nnodes 2 --node-rank $(WORKER_INDEX)"},
		...
		},
		{
			...
			initialArgs:       []string{},
			expectedCommand:   []string{"sh", "-c"},
			expectedArgs:      []string{"exec python3 -m dingo.sglang --dist-init-addr $(LEADER_HOST):29500 --nnodes 2 --node-rank $(WORKER_INDEX)"},
...
func TestSGLangBackend_ShellCommandInjection(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			initialArgs:       []string{"python -m dingo.sglang"},
			expectedArgs:      []string{"python -m dingo.sglang"},
		...
		},
		{
			...
			initialCommand:    []string{"sh", "-c"},
			initialArgs:       []string{"python -m dingo.sglang"},
			expectedArgs:      []string{"python -m dingo.sglang --dist-init-addr $(GROVE_PCSG_NAME)-$(GROVE_PCSG_INDEX)-test-service-ldr-0.$(GROVE_HEADLESS_SERVICE):29500 --nnodes 2 --node-rank 0"},
		...
		},
		{
			...
			multinodeDeployer: &GroveMultinodeDeployer{},
			initialCommand:    []string{"sh", "-c"},
			initialArgs:       []string{"echo blah | wc -l && python -m dingo.sglang && ls -al"},
			expectedArgs:      []string{"echo blah | wc -l && python -m dingo.sglang --dist-init-addr $(GROVE_PCSG_NAME)-$(GROVE_PCSG_INDEX)-test-service-ldr-0.$(GROVE_HEADLESS_SERVICE):29500 --nnodes 2 --node-rank 0 && ls -al"},
		...
		},
		{
			...
			initialCommand:    []string{"sh", "-c"},
			initialArgs:       []string{"python -m dingo.sglang"},
			expectedArgs:      []string{"python -m dingo.sglang --dist-init-addr $(GROVE_PCSG_NAME)-$(GROVE_PCSG_INDEX)-test-service-ldr-0.$(GROVE_HEADLESS_SERVICE):29500 --nnodes 3 --node-rank $((GROVE_PCLQ_POD_INDEX + 1))"},
		...
		},
		{
			...
			initialCommand:    []string{"sh", "-c"},
			initialArgs:       []string{"python -m dingo.sglang"},
			expectedArgs:      []string{"python -m dingo.sglang --dist-init-addr $(LWS_LEADER_ADDRESS):29500 --nnodes 2 --node-rank 0"},
		...
		},
		{
			...
			multinodeDeployer: &GroveMultinodeDeployer{},
			initialCommand:    []string{"sh", "-c"},
			initialArgs:       []string{"python -m dingo.sglang | tee /tmp/log"},
			expectedArgs:      []string{"python -m dingo.sglang --dist-init-addr $(GROVE_PCSG_NAME)-$(GROVE_PCSG_INDEX)-test-service-ldr-0.$(GROVE_HEADLESS_SERVICE):29500 --nnodes 2 --node-rank 0 | tee /tmp/log"},
		...
		},
		{
			...
			multinodeDeployer: &GroveMultinodeDeployer{},
			initialCommand:    []string{"sh", "-c"},
			initialArgs:       []string{"echo start", "python -m dingo.sglang", "echo done"},
			expectedArgs:      []string{"echo start", "python -m dingo.sglang --dist-init-addr $(GROVE_PCSG_NAME)-$(GROVE_PCSG_INDEX)-test-service-ldr-0.$(GROVE_HEADLESS_SERVICE):29500 --nnodes 2 --node-rank 0", "echo done"},
		...
		},
		{
			...
			multinodeDeployer: &GroveMultinodeDeployer{},
			initialCommand:    []string{"sh", "-c"},
			initialArgs:       []string{"python -m dingo.sglang", "python -m dingo.sglang --other-flags"},
			expectedArgs:      []string{"python -m dingo.sglang --dist-init-addr $(GROVE_PCSG_NAME)-$(GROVE_PCSG_INDEX)-test-service-ldr-0.$(GROVE_HEADLESS_SERVICE):29500 --nnodes 2 --node-rank 0", "python -m dingo.sglang --other-flags"},
