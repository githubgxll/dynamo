...
from dingo.common.backend.engine import DiffusionEngine, EngineConfig
...
def _make_handler(modality: str, generator: Any, config: Any, fs: Any) -> Any:
    ...
    if modality == _IMAGE:
        from dingo.sglang.request_handlers import ImageDiffusionWorkerHandler

        return ImageDiffusionWorkerHandler(generator, config, publisher=None, fs=fs)
    if modality == _VIDEO:
        from dingo.sglang.request_handlers import VideoGenerationWorkerHandler
    ...
    async def from_args(
        ...
        if dynamo_args.image_diffusion_worker:
            ...
            raise ValueError(
                ...
                "--video-generation-worker (use dingo.sglang.unified_main for "
