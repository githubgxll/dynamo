...
class GenerateRequest(TypedDict, total=False):
    ...
    read them via ``dingo.common.backend.disagg`` helpers.
    ...
    :func:`dingo.common.backend.multimodal.require_encoder_result`. All
...
class GenerateChunk(TypedDict, total=False):
    ...
    :mod:`dingo.common.backend.logprobs`.
    ...
    :func:`dingo.common.backend.multimodal.encoder_terminal_chunk`.
    """
...
class BaseEngine(ABC):
    ...
    async def register_prometheus(self, metrics: "EngineMetrics") -> None:
        ...
        no-op. See :mod:`dingo.common.backend.metrics` for helpers. Do not
...
# dingo.common.protocols.image_protocol (videos: video_protocol).
