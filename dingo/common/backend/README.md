...
```bash
python -m dingo.common.backend.sample_main \
...
```python
from dingo.common.backend import LLMEngine, EngineConfig, LlmRegistration, WorkerConfig
...
`dingo.common.backend.disagg` ships small utilities engines can call
...
`dingo.common.backend.telemetry` facade, which mirrors the OpenTelemetry
...
- **Logprobs** — selected-token + top-k logprob extraction and
  streaming, sourced from `dingo.common.backend.logprobs` and used by
