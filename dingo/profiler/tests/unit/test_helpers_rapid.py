...
try:
    from dingo.profiler.rapid import _run_default_sim, _run_naive_fallback
...
class TestRunDefaultSim:
    ...
    def test_returns_required_keys(self):
        dgdr = _make_dgdr()
        with (
            patch("dingo.profiler.rapid.build_default_task_configs", return_value={}),
            patch(
                "dingo.profiler.rapid._execute_task_configs",
                return_value=self._execute_return(),
            ),
            patch(
                "dingo.profiler.rapid._generate_dgd_from_pick",
    ...
    def test_load_match_passes_load_kwargs(self):
        ...
        with (
            patch("dingo.profiler.rapid.build_default_task_configs", return_value={}),
            patch(
                "dingo.profiler.rapid._execute_task_configs", side_effect=fake_execute
            ),
