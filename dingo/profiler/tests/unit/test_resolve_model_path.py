...
try:
    from dingo.profiler.rapid import (
        _generate_dgd_from_pick,
        _run_autoscale_sim,
        _run_default_sim,
    )
    from dingo.profiler.thorough import run_thorough
...
class TestRapidResolvesModelPath:
    ...
    def test_default_sim_uses_local_path_when_pvc_mounted(self, tmp_path):
        ...
        with (
            patch(
                "dingo.profiler.rapid.build_default_task_configs", return_value={}
            ) as mock_build,
            patch(
                "dingo.profiler.rapid._execute_task_configs",
    ...
    def test_autoscale_sim_uses_local_path_when_pvc_mounted(self, tmp_path):
        ...
        with (
            patch("dingo.profiler.rapid.TaskConfig") as mock_task_config,
            patch("dingo.profiler.rapid.TaskRunner", return_value=runner),
            patch("dingo.profiler.rapid._generate_dgd_from_pick", return_value=None),
        ):
            _run_autoscale_sim(
...
class TestGenerateDgdKeepsServedModelName:
    ...
    def _capture_generate(self, dgdr, cfg) -> MagicMock:
        ...
        with (
            patch(
                "dingo.profiler.rapid.task_config_to_generator_config",
                return_value=cfg,
            ),
            patch(
                "dingo.profiler.rapid.generate_backend_artifacts",
...
class TestThoroughResolvesModelPath:
    ...
    async def _capture_enumerate(self, dgdr, output_dir) -> MagicMock:
        ...
        with (
            patch(
                "dingo.profiler.thorough.enumerate_profiling_configs",
                return_value=([], []),
            ) as mock_enumerate,
            patch(
                "dingo.profiler.thorough._benchmark_prefill_candidates",
                new=AsyncMock(return_value=pd.DataFrame()),
            ),
            patch(
                "dingo.profiler.thorough._benchmark_decode_candidates",
                new=AsyncMock(return_value=pd.DataFrame()),
            ),
        ):
            await run_thorough(
    ...
    async def _capture_task_config(self, dgdr, output_dir) -> MagicMock:
        ...
        with (
            ...
            patch(
                "dingo.profiler.thorough._pick_thorough_best_config",
                return_value={"best_config_df": pd.DataFrame()},
            ),
            patch("dingo.profiler.thorough.TaskConfig") as mock_task_config,
            patch(
                "dingo.profiler.thorough._generate_dgd_from_pick",
                return_value=None,
            ),
        ):
            await run_thorough(
