...
try:
    ...
    from dingo.profiler.utils.config_modifiers.protocol import (
        BaseConfigModifier,
        apply_dgd_overrides,
    )
    from dingo.profiler.utils.defaults import EngineType, SearchStrategy
    from dingo.profiler.utils.dgdr_v1beta1_types import (
        DynamoGraphDeploymentRequestSpec,
        OverridesSpec,
    )
    from dingo.profiler.utils.profile_common import ProfilerOperationalConfig
except ImportError:
...
def test_vllm_mamba_align_raises_max_num_batched_tokens() -> None:
    ...
    with patch(
        "dingo.profiler.utils.config_modifiers.vllm.get_mamba_cache_align_block_size",
        return_value=8320,
    ):
...
def test_apply_dgd_overrides_missing_service_skipped_with_warning(caplog) -> None:
    ...
    with caplog.at_level(
        logging.WARNING, logger="dingo.profiler.utils.config_modifiers.protocol"
    ):
        result = apply_dgd_overrides(dgd_config, overrides)
...
async def test_run_profile_applies_dgd_overrides_before_interpolation(
    ...
    with (
        ...
        patch(
            "dingo.profiler.profile_sla.check_model_hardware_support",
            return_value=True,
        ),
        patch(
        ...
        patch(
            "dingo.profiler.profile_sla._execute_strategy",
            new=AsyncMock(
                return_value=(
                    pick_result,
                    PickedParallelConfig(),
        ...
        patch("dingo.profiler.profile_sla.needs_profile_data", return_value=True),
        patch(
            "dingo.profiler.profile_sla.run_interpolation",
            new=_fake_interpolation,
        ),
        patch(
            "dingo.profiler.profile_sla.assemble_final_config",
            return_value=copy.deepcopy(base_dgd),
        ),
        patch("dingo.profiler.profile_sla._write_final_output", return_value=True),
        patch("dingo.profiler.profile_sla.write_profiler_status"),
        patch(
            "dingo.profiler.profile_sla.cleanup_remaining_deployments",
            new=AsyncMock(),
        ),
    ):
        with caplog.at_level(
            logging.WARNING,
            logger="dingo.profiler.utils.config_modifiers.protocol",
        ):
