...
def _make_thorough_patches(backend: str = "trtllm"):
    ...
    return [
        patch(
            "dingo.profiler.thorough.DynamoDeploymentClient",
            side_effect=lambda **kw: _mock_deployment_client(),
        ),
        patch("dingo.profiler.thorough.get_prefill_ttft", return_value=50.0),
        patch(
            "dingo.profiler.thorough.get_decode_itl_and_thpt_per_gpu",
            return_value=(8.0, 125.0),
        ),
        patch("dingo.profiler.thorough.get_num_request_range", return_value=[1, 4, 8]),
        patch(
            "dingo.profiler.thorough.pick_decode_component",
...
class TestThoroughMocked:
    ...
    def test_thorough_planner_thorough_sweep(self, tmp_path):
        ...
        interp_patches = [
            patch(
                "dingo.profiler.interpolation.DynamoDeploymentClient",
                side_effect=lambda **kw: _mock_deployment_client(),
            ),
            patch(
                "dingo.profiler.interpolation.profile_prefill",
                side_effect=mock_profile_prefill,
            ),
            patch(
                "dingo.profiler.interpolation.profile_decode",
