...
mock_vllm_cli = make_cli_args_fixture("dingo.vllm")
...
def test_headless_namespace_has_required_fields(mock_vllm_cli):
    ...
    from dingo.vllm.main import build_headless_namespace
...
def test_should_prefetch_model_for_default_load_format():
    from dingo.vllm.main import should_prefetch_model
...
def test_should_register_model_fetch_weights_for_default_load_format():
    from dingo.vllm.main import should_register_model_ignore_weights
...
class TestGetHostIp:
    ...
    def test_hostname_link_local_falls_through_to_udp(self):
        ...
        with (
            patch(
                "dingo.vllm.args._try_hostname_resolution", return_value="169.254.1.1"
            ),
            patch("dingo.vllm.args._try_udp_connect") as mock_udp,
        ):
            mock_udp.side_effect = lambda family, target: (
    ...
    def test_ipv6_fallback(self):
        ...
        with (
            patch("dingo.vllm.args._try_hostname_resolution", return_value=None),
            patch("dingo.vllm.args._try_udp_connect") as mock_udp,
        ):
            mock_udp.side_effect = lambda family, target: (
    ...
    def test_all_fail_raises_runtime_error(self):
        ...
        with (
            patch("dingo.vllm.args._try_hostname_resolution", return_value=None),
            patch("dingo.vllm.args._try_udp_connect", return_value=None),
        ):
...
class TestEnsureSideChannelHost:
    def test_preserves_existing_env_var(self, monkeypatch):
        ...
        with patch("dingo.vllm.args.get_host_ip") as mock_get:
            ensure_side_channel_host()
    ...
    def test_sets_env_var_on_successful_detection(self, monkeypatch):
        ...
        with patch("dingo.vllm.args.get_host_ip", return_value="10.0.0.5"):
            ensure_side_channel_host()
...
class TestVllmOmniOptionalDependency:
    def test_dynamo_vllm_main_importable_without_vllm_omni(self):
        """dingo.vllm.main must import cleanly even when vllm_omni is absent.
        ...
        """
        # Save and evict any already-cached vllm_omni and dingo.vllm.omni modules
        saved = {
            k: sys.modules.pop(k)
            for k in list(sys.modules)
            if k == "vllm_omni"
            or k.startswith("vllm_omni.")
            or k == "dingo.vllm.main"
            or k.startswith("dingo.vllm.omni")
        }
        ...
        try:
            import dingo.vllm.main  # noqa: F401
        except ImportError as e:
            pytest.fail(f"dingo.vllm.main has a hard dependency on vllm_omni: {e}")
        finally:
            sys.modules.pop("vllm_omni", None)
            # Remove any modules imported during this test
            for mod in list(sys.modules):
                if mod == "dingo.vllm.main" or mod.startswith("dingo.vllm.omni"):
...
class TestBenchmarkConfig:
    ...
    def test_benchmark_config_defaults(self):
        from dingo.vllm.instrumented_scheduler import BenchmarkConfig
...
async def test_health_check_decode_opts_out_with_warning():
    ...
    with patch("dingo.vllm.llm_engine.logger") as mock_logger:
