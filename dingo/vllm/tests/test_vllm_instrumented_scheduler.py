...
# (which shadows the real ``vllm`` with the ``dingo.vllm`` submodule for any
# later bare ``import vllm``). Mirrors the pattern in ``test_vllm_unit.py``,
# which imports ``dingo.vllm.args`` at module level for the same reason.
...
from dingo.vllm.instrumented_scheduler import (  # noqa: E402
