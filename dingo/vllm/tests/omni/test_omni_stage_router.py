...
def _patched_generate(router, request, request_id="req-1", request_type="chat"):
    return (
        patch(
            "dingo.vllm.omni.stage_router.parse_request_type",
            return_value=(None, request_type),
        ),
        patch("dingo.vllm.omni.stage_router.uuid.uuid4", return_value=request_id),
    )
...
def test_router_loads_stage_configs_from_model_deploy_config():
    ...
    with (
        patch(
            "dingo.vllm.omni.stage_router.load_and_resolve_stage_configs",
            return_value=("/deploy/glm_image.yaml", stage_configs),
        ) as load_and_resolve_stage_configs,
        patch("dingo.vllm.omni.stage_router.OutputFormatter") as output_formatter,
    ):
...
async def test_generate_concurrent_requests_have_independent_connector_refs():
    ...
    async def run_one(request_id):
        with patch(
            ...
            with patch(
                "dingo.vllm.omni.stage_router.uuid.uuid4", return_value=request_id
            ):
                with patch.object(
                    stage_router, "shm_deserialize", return_value=SimpleNamespace()
                ):
...
async def test_generate_delegates_formatting_to_output_formatter():
    ...
    with patch.object(stage_router, "shm_deserialize", return_value=fake_result):
        with patch(
            ...
            with patch(
                "dingo.vllm.omni.stage_router.uuid.uuid4", return_value="req-fmt"
            ):
...
async def test_generate_yields_error_when_no_shm_meta():
    ...
    with patch(
        ...
        with patch("dingo.vllm.omni.stage_router.uuid.uuid4", return_value="r"):
...
async def test_generate_forwards_raw_request_to_stage0():
    ...
    with patch(
        ...
        with patch("dingo.vllm.omni.stage_router.uuid.uuid4", return_value="req-raw"):
            with patch.object(
                stage_router, "shm_deserialize", return_value=SimpleNamespace()
            ):
