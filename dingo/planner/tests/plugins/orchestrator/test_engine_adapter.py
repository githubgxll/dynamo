...
from dingo.planner.plugins.merge.types import ChainAugmentOutcome
...
from dingo.planner.plugins.types import ComponentTarget, ScalingProposal
...
def _agg_config_with_in_process_plugin() -> PlannerConfig:
    from dingo.planner.plugins.registry.config import (
        InProcessPluginSpec,
        ...
        plugin_registration=PluginRegistrationConfig(
            in_process_plugins=[
                InProcessPluginSpec.model_validate(
                    {
                        "module": "dingo.planner.tests.plugins.orchestrator._fake_in_process_plugin",
                        "class": "FakePlugin",
...
async def test_builtin_load_propose_refreshes_throughput_bound_before_load():
    from dingo.planner.plugins.builtins.local_planner import BuiltinLoadPropose
...
def test_tick_input_to_context_maps_observations_and_fpm():
    ...
    from dingo.common.forward_pass_metrics import decode as _fpm_decode
...
def test_orchestrator_path_honours_configured_protocol_version_range():
    ...
    gateway).
    """
    from dingo.planner.config.planner_config import PluginRegistrationConfig
