...
from dingo.planner.connectors.remote_client import RemotePlannerClient
from dingo.planner.errors import DeploymentValidationError, EmptyTargetReplicasError
...
async def test_connector_initialization(connector, connector_runtime):
    ...
    with patch(
        "dingo.planner.connectors.global_planner.RemotePlannerClient"
...
async def test_connector_set_replicas_rejected(connector):
    ...
    with patch.dict(
        ...
        with patch("dingo.planner.connectors.global_planner.logger") as mock_logger:
...
def test_connector_model_name_and_predicted_load(connector_runtime):
    ...
    with patch(
        "dingo.planner.connectors.global_planner.KubernetesConnector",
