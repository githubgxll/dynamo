...
from dingo.planner.errors import DynamoGraphDeploymentNotFoundError
...
@pytest.fixture
def mock_config():
    with patch("dingo.planner.connectors.kubernetes_api.config") as mock:
...
def mock_custom_api():
    with patch(
        "dingo.planner.connectors.kubernetes_api.client.CustomObjectsApi"
    ) as mock:
...
def mock_namespace():
    with patch(
        "dingo.planner.connectors.kubernetes_api.get_current_k8s_namespace",
