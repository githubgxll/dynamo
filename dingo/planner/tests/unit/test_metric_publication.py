...
from dingo.planner.core.base import NativePlannerBase
...
from dingo.planner.monitoring.planner_metrics import PREFIX, PlannerPrometheusMetrics
...
def _make_planner(prometheus_enabled: bool = True) -> NativePlannerBase:
    ...
    """
    with patch(
        "dingo.planner.core.base.PlannerPrometheusMetrics"
    ) as mock_metrics, patch("dingo.planner.core.base.start_http_server"), patch(
        "dingo.planner.connectors.kubernetes.KubernetesAPI"
    ), patch.dict(
...
class TestPlannerPrometheusMetricsHasSlaTargetGauges:
    ...
    def test_metrics_object_has_sla_target_gauge_attributes(self):
        ...
        with patch(
            "dingo.planner.monitoring.planner_metrics.Gauge"
        ) as mock_gauge, patch("dingo.planner.monitoring.planner_metrics.Enum"):
