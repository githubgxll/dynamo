...
planner stubs are explicitly negated with `!dingo/planner/plugins/proto/v1/*`.

Regenerate locally when you edit `plugin.proto`:

```bash
# Regenerate all three stubs (run from the repository root)
python -m grpc_tools.protoc \
    --python_out=. --grpc_python_out=. --pyi_out=. --proto_path=. \
    dingo/planner/plugins/proto/v1/plugin.proto
...
for f in dingo/planner/plugins/proto/v1/plugin_pb2.py \
         dingo/planner/plugins/proto/v1/plugin_pb2_grpc.py \
         dingo/planner/plugins/proto/v1/plugin_pb2.pyi; do
