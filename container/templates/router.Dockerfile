...
# The default --dyn-chat-processor is "auto": if sglang is installed in the
# image it uses the sglang processor (full feature parity with workers);
# otherwise it falls back to the Rust-native dynamo processor.
...
CMD ["--namespace-prefix", "glm51-mixed-", "--discovery-backend", "etcd", "--request-plane", "tcp", "--event-plane", "nats", "--http-host", "0.0.0.0", "--http-port", "8000", "--router-mode", "round-robin", "--router-min-initial-workers", "1", "--trust-remote-code", "--metrics-prefix", "sg_glm51_mixed_dingo"]
