...
This is distinct from the [Standalone Router](../../../dingo/router/README.md), which is a full routing service. The standalone indexer provides only the indexing and query layer without routing logic.

For Dynamo-native remote indexing, use `--serve-indexer` on `dingo.frontend` or `dingo.router` and `--use-remote-indexer` on consumers instead. That request-plane service reuses the router's existing event ingestion and recovery machinery; it is not implemented by `dynamo.indexer`.
...
- **[Standalone Router](../../../dingo/router/README.md)**: Full routing service (routes requests to workers)
