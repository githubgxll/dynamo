...
<Note>
- **etcd** is optional but is the default local discovery backend. File-based discovery is also available (see `python -m dingo.vllm --help` for `--discovery-backend` options).
