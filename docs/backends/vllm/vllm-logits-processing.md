...
The unified vLLM engine threads logits processors through the shared spec layer in `dingo.common.backend.engine` and the per-backend realizer at `dingo.vllm.logits_processing.adapter`:
